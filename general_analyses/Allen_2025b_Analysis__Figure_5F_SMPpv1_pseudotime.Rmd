---
title: "Allen_2025b_Analysis__Figure_5F_SMPpv1_pseudotime"
...
```{r avg-exp}
avg_exp <- AverageExpression(object = SMPpv1_seu, 
                             assays = "RNA", 
                             slot = "counts",
...
```{r}
Idents(SMPpv1_seu) <- "RNA_snn_res.4"
all_imp_cells <- WhichCells(object = SMPpv1_seu, idents = all_imp_cluster)
head(all_imp_cells)
```
...
```{r}
all_imp_metadata <- FetchData(object = SMPpv1_seu, vars = "RNA_snn_res.4") %>% 
    rownames_to_column("cell_id") %>% 
    mutate(all_imp = if_else(cell_id %in% all_imp_cells, "yes", "no"))

SMPpv1_seu <- AddMetaData(object = SMPpv1_seu, metadata = all_imp_metadata$all_imp, col.name = "imp_cells")
```
...
```{r}
SMPpv1_split <- SplitObject(object = SMPpv1_seu, split.by = "experiment")
SMPpv1_split
```
...
for (i in seq_along(SMPpv1_split)) {
    SMPpv1_split[[i]] <- FindVariableFeatures(object = SMPpv1_split[[i]],
...
var_features <- SelectIntegrationFeatures(object.list = SMPpv1_split, nfeatures = n_var_genes)
...
## remove B-H1/2-, mab-21- cluster

```{r}
SMPpv1_seu
Idents(SMPpv1_seu) <- "RNA_snn_res.4"
cells_remove <- WhichCells(object = SMPpv1_seu, idents = 16)
SMPpv1_seu <- subset(x = SMPpv1_seu, cells = cells_remove, invert = TRUE)
SMPpv1_seu
```
...
<!-- SMPpv1_split[["sleep"]] <- NULL -->
<!-- SMPpv1_split[["species"]] <- NULL -->

<!-- SMPpv1_sub <- merge(SMPpv1_split[[1]], -->
<!-- 		                unlist(SMPpv1_split[2:length(SMPpv1_split)]), -->
<!-- 		                merge.data = TRUE) -->
...
<!-- expression_matrix <- SMPpv1_sub@assays[["RNA"]]@counts -->
<!-- cell_metadata <- SMPpv1_sub@meta.data -->
<!-- gene_annotation <-  data.frame(gene = rownames(SMPpv1_sub)) %>% -->
<!--     mutate(gene_short_name = gene) %>% -->
<!--     column_to_rownames("gene") -->

<!-- SMPpv1_cds <- new_cell_data_set(expression_matrix, -->
...
expression_matrix <- SMPpv1_seu@assays[["RNA"]]@counts
cell_metadata <- SMPpv1_seu@meta.data
gene_annotation <-  data.frame(gene = rownames(SMPpv1_seu)) %>%
    mutate(gene_short_name = gene) %>%
    column_to_rownames("gene")
...
get_earliest_principal_node <- function(SMPpv1_cds, early_born="yes"){
  cell_ids <- which(colData(SMPpv1_cds)[, "imp_cells"] == early_born)

  closest_vertex <- SMPpv1_cds@principal_graph_aux[["UMAP"]]$pr_graph_cell_proj_closest_vertex
  closest_vertex <- as.matrix(closest_vertex[colnames(SMPpv1_cds), ])
  root_pr_nodes <- igraph::V(principal_graph(SMPpv1_cds)[["UMAP"]])$name[as.numeric(names(which.max(table(closest_vertex[cell_ids,]))))]
...
# genes_use <- dplyr::setdiff(VariableFeatures(SMPpv1_seu), iso_genes_remove)
...
SMPpv1_cds <- preprocess_cds(SMPpv1_cds, method = "PCA",
                      num_dim = num_pcs,
                      norm_method = "log",
                      use_genes = genes_use,
                      verbose = set_verbose
...
SMPpv1_cds <- align_cds(SMPpv1_cds,
...
SMPpv1_cds <- reduce_dimension(SMPpv1_cds,
                        max_components = 2,
...
SMPpv1_cds <- cluster_cells(SMPpv1_cds,
                     reduction_method = "UMAP",
                     k = num_neighbours,
...
SMPpv1_cds <- learn_graph(SMPpv1_cds,
                   use_partition = FALSE,
                   verbose = set_verbose
                   )


SMPpv1_cds <- order_cells(SMPpv1_cds, root_pr_nodes=get_earliest_principal_node(SMPpv1_cds))
...
plot(plot_cells(SMPpv1_cds_female,
           color_cells_by = "pseudotime",
           label_cell_groups = FALSE,
           label_leaves = FALSE,
           label_branch_points = FALSE,
...
plot(plot_cells(SMPpv1_cds_male,
           color_cells_by = "pseudotime",
           label_cell_groups = FALSE,
           label_leaves = FALSE,
           label_branch_points = FALSE,
...
```{r sig-genes}
modulated_sig_SMPpv1_genes <- graph_test(SMPpv1_cds, neighbor_graph = "principal_graph", cores = 10)
sig_SMPpv1_genes <- row.names(subset(modulated_sig_SMPpv1_genes, q_value < 1e-50 & morans_I > 0.2)) %>% dplyr::setdiff(iso_genes_remove) 
sig_SMPpv1_genes %>% sort()
```

### plot

```{r}
pt_SMPpv1_matrix <- exprs(SMPpv1_cds)[match(sig_SMPpv1_genes,rownames(rowData(SMPpv1_cds))),order(pseudotime(SMPpv1_cds))]
pt_SMPpv1_matrix <- t(apply(pt_SMPpv1_matrix,1,function(x){smooth.spline(x, df = smooth_spline_df)$y}))
pt_SMPpv1_matrix <- t(apply(pt_SMPpv1_matrix,1,function(x){(x - mean(x))/sd(x)}))
rownames(pt_SMPpv1_matrix) <- sig_SMPpv1_genes
```
...
```{r}
sig_SMPpv1_tf_genes <- dplyr::intersect(sig_SMPpv1_genes, tfs_symbol$gene_symbol)
sig_SMPpv1_tf_genes
```


```{r}
tf_pt_SMPpv1_matrix <- exprs(SMPpv1_cds)[match(sig_SMPpv1_tf_genes,rownames(rowData(SMPpv1_cds))),order(pseudotime(SMPpv1_cds))]
tf_pt_SMPpv1_matrix <- t(apply(tf_pt_SMPpv1_matrix,1,function(x){smooth.spline(x, df = smooth_spline_df)$y}))
tf_pt_SMPpv1_matrix <- t(apply(tf_pt_SMPpv1_matrix,1,function(x){(x - mean(x))/sd(x)}))
rownames(tf_pt_SMPpv1_matrix) <- sig_SMPpv1_tf_genes
```
...
genes_of_interest <- sig_SMPpv1_tf_genes # Replace with actual gene names

# Extract expression data
expr_data <- as.data.frame(exprs(SMPpv1_cds)[genes_of_interest, ])

# Get metadata including pseudotime and your variable of interest
cell_metadata <- colData(SMPpv1_cds) %>%
    as.data.frame() %>%
    rownames_to_column("cell_id") %>%
    select(cell_id, sex) %>% 
    left_join(pseudotime(SMPpv1_cds) %>%
                as.data.frame() %>%
                rownames_to_column("cell_id") %>%
...
expr_data <- expr_data %>%
    t() %>%
    as.data.frame() %>%
    mutate(cell_id = colnames(SMPpv1_cds),
...
sex_pt_SMPpv1_matrix <- exprs(SMPpv1_cds)[match(genes_sub,rownames(rowData(SMPpv1_cds))),order(pseudotime(SMPpv1_cds))]
sex_pt_SMPpv1_matrix <- t(apply(sex_pt_SMPpv1_matrix,1,function(x){smooth.spline(x, df = smooth_spline_df)$y}))
sex_pt_SMPpv1_matrix <- t(apply(sex_pt_SMPpv1_matrix,1,function(x){(x - mean(x))/sd(x)}))
rownames(sex_pt_SMPpv1_matrix) <- genes_sub 
```
...
df <- expr_data %>% 
    ...
    rownames_to_column("cell_id") %>% 
    left_join(FetchData(object = SMPpv1_seu, 
                        vars = "experiment") %>% 
                  rownames_to_column("cell_id"),
              by = "cell_id") %>% 
...
sex_pt_SMPpv1_matrix <- normalized_counts(SMPpv1_cds, norm_method = "log")[match(genes_sub,rownames(rowData(SMPpv1_cds))),order(pseudotime(SMPpv1_cds))]
sex_pt_SMPpv1_matrix <- t(apply(sex_pt_SMPpv1_matrix,1,function(x){smooth.spline(x, df = smooth_spline_df)$y}))
rownames(sex_pt_SMPpv1_matrix) <- genes_sub 
```
