---
title: "Allen_2025a_Analysis__Figure_4_ALad1_subclustering"
...
cells_to_keep_file <- "../../proj136/analyses/cell_lists/subclustering_ALad1_metablob_cells_to_keep--round2.csv"

if (file.exists(cells_to_keep_file)) {
    cells_keep <- read_csv(file = cells_to_keep_file)
    for (i in seq_along(opn_list_clean)) {
        opn_list_clean[[i]] <-subset(opn_list_clean[[i]], cells = cells_keep$cell_id, invert = FALSE)
    }
    opn_list_clean
}
```
...
```{r}
var_features <- SelectIntegrationFeatures(object.list = opn_list_clean, nfeatures = n_var_gene)
...
var_features <- var_features[1:min(1000, length(var_features))]
```
...
```{r}
FetchData(object = opn_seu, vars = "experiment") %>% dplyr::count(experiment) %>% arrange(desc(n))
```

```{r}
FetchData(object = opn_seu, vars = "batch") %>% dplyr::count(batch) %>% arrange(desc(n))
```
...
opn_seu <- ScaleData(object = opn_seu,
                        assay = "RNA",
                        vars.to.regress = c("experiment", "sex", "nCount_RNA", "percent_mt", "percent_hsp") #  "orig.ident", 
                     )
...
opn_seu <- RunHarmony(opn_seu,
                    group.by.vars = c("experiment"), #, "sex", "nCount_RNA", "orig.ident", "percent_mt", "percent_hsp"),
					# lambda = c(1,1),
                    assay.use = "RNA")
invisible(gc())
```
...
```{r fig.width=8, fig.height=8}
DimPlot(object = opn_seu, reduction = paste0("umap_40pcs"), group.by = "experiment") + coord_fixed() + ggtitle("ALad1 hemilineage")
```
