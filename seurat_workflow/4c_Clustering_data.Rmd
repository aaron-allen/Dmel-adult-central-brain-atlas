...
<!-- ```{r calc-phylo, fig.width = 3, fig.height = 8} -->
<!-- start_time <- Sys.time() -->
<!-- if (integration == "cca") { -->
...
<!-- integrated <- BuildClusterTree(integrated, dims = 1:curr_pc, reduction = set_reduction, assay = assay_use) -->
<!-- plot.phylo(integrated@tools$BuildClusterTree, cex = .5, no.margin = TRUE) -->
<!-- process_time <- stop_the_clock("calc_phylo") -->
<!-- ``` -->



<!-- ```{r collapse-clusters, warning = FALSE, message = FALSE} -->
<!-- start_time <- Sys.time() -->
<!-- # if (parallelize) { -->
<!-- #     plan(futures_plan, workers = futures_n_cores) -->
<!-- #     options(future.globals.maxSize = futures_ram * 1024 * 1024^2) -->
...
<!-- ### I haven't cleaned up this code much at all ... -->
<!-- ### but added some parameterization and options to the FindMarkers function. -->

<!-- ### note: this approach assumes that the cluster numbers start at 0 (and not 1) -->

<!-- phylo_table <- as_tibble(integrated@tools$BuildClusterTree$edge) %>% -->
<!--                   mutate(lengths = integrated@tools$BuildClusterTree$edge.length, -->
<!--                          Cluster = V2 - 1 -->
<!--                          ) -->
<!-- integrated$merged_clusters <- Idents(integrated) -->


<!-- nclust_orig <- length(levels(integrated)) -->
<!-- full <- FALSE -->
<!-- clear <- c() -->
<!-- round <- 1 -->
...
<!--         if (phylo_table[i, ]$V2 <= nclust_orig && -->
<!--            phylo_table[i + 1, ]$V2 <= nclust_orig && -->
<!--            phylo_table[i, ]$V1 == phylo_table[i + 1, ]$V1 && -->
<!--            !phylo_table[i, ]$Cluster %in% clear) { -->


<!--             ident_1_size <- length(WhichCells(object = integrated, idents = phylo_table[i, ]$Cluster)) -->
<!--             ident_2_size <- length(WhichCells(object = integrated, idents = phylo_table[i + 1, ]$Cluster)) -->

<!--             if (ident_1_size > min_cluster_size && -->
<!--                 ident_2_size > min_cluster_size) { -->
<!--                 #cat("Testing clusters", phylo_table[i, ]$Cluster, "and", phylo_table[i+1, ]$Cluster, "...\n") -->
<!--                 markers_i <- FindMarkers(object = integrated, -->
<!--                                         ident.1 = phylo_table[i, ]$Cluster, -->
<!--                                         ident.2 = phylo_table[i + 1, ]$Cluster, -->
<!--                                         test.use = test_use, -->
<!--                                         vars.to.regress = vars_to_regress, -->
<!--                                         assay = "RNA", -->
<!--                                         min.pct = min_pct) -->
<!--                 # add check for logFC vs log2FC -->
<!--                 n_sig <- markers_i %>% -->
<!--                             rownames_to_column("Gene") %>% -->
<!--                             dplyr::filter(abs(avg_log2FC) >= avg_logFC_thres, -->
<!--                                         p_val_adj < p_val_adj_thres, -->
<!--                                         !grepl("(lncRNA|tRNA|^mt:|rRNA)", Gene) -->
<!--                                         ) %>% -->
<!--                             dplyr::summarise(n()) %>% -->
<!--                             pull() -->
<!--             } else { -->
...
<!--             if (n_sig < lim_n_sig) { -->
<!--                 #cat("Only", n_sig, "good markers, the clusters are merged.\n\n") -->
...
<!--                 integrated$merged_clusters[integrated$merged_clusters == other_clus] <- min_clus -->
<!--                 phylo_table[i - 1, ] <- list(phylo_table[i - 1, ]$V1, min_clus + 1, sum(phylo_table[c(i, i - 1), ]$lengths), min_clus) -->
<!--                 phylo_table <- phylo_table[-c(i, i + 1), ] -->
<!--                 full <- FALSE -->
<!--                 i <- i - 1 -->
<!--             } else { -->
<!--                 #cat("These clusters are well supported with", n_sig, "markers.\n\n") -->
...
<!-- # Save info in Seurat object. The tree file is not in a format that can be printed, but it could be edited if necessary. -->
<!-- integrated@tools$phylo_table <- phylo_table -->
...
<!-- #     plan("sequential") -->
<!-- # } -->
<!-- process_time <- stop_the_clock("collapse_clusters") -->
<!-- ``` -->

<!-- ## Show merged clusters -->

<!-- ```{r merged-clusters, fig.height = 8, fig.width = 8} -->
<!-- start_time <- Sys.time() -->
<!-- integrated@meta.data[, c("seurat_clusters", "merged_clusters")] %>% -->
<!--     mutate(seurat_clusters = fct_relevel(seurat_clusters, as.character(sort(as.numeric(unique(.$seurat_clusters))))), -->
<!--            merged_clusters = fct_relevel(merged_clusters, as.character(sort(as.numeric(unique(.$merged_clusters))))) -->
<!--         ) %>% -->
<!--     ggplot(aes(x = merged_clusters, y = seurat_clusters)) + -->
<!--         geom_point(shape = 4) + -->
<!--         labs(x = "Merged clusters", y = "Original clusters") + -->
<!--         theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5), -->
<!--               axis.text = element_text(size = 7)) -->
<!-- process_time <- stop_the_clock("merged_clusters") -->
<!-- ``` -->

<!-- ## Assign new cluster identity and plot -->

<!-- ```{r plot-merged, fig.asp = .5} -->
<!-- start_time <- Sys.time() -->
<!-- Idents(integrated) <- integrated$seurat_clusters -->
<!-- p1 <- DimPlot(integrated, reduction = paste0("umap_", curr_pc, "pcs"), label = TRUE, pt.size = min(4, pt_size), raster = FALSE) + -->
<!--         NoLegend() + -->
<!--         NoAxes() + -->
<!--         ggtitle("Original clusters") + -->
<!--         theme(plot.title = element_text(size = 10, hjust = .5)) -->
<!-- Idents(integrated) <- integrated$merged_clusters -->
<!-- p2 <- DimPlot(integrated, reduction = paste0("umap_", curr_pc, "pcs"), label = TRUE, pt.size = min(4, pt_size), raster = FALSE) + -->
<!--         NoLegend() + -->
<!--         NoAxes() + -->
<!--         ggtitle("Merged clusters") + -->
<!--         theme(plot.title = element_text(size = 10, hjust = .5)) -->
<!-- plot_grid(p1, p2, ncol = 2, labels = "AUTO") -->
<!-- process_time <- stop_the_clock("plot_merged") -->
<!-- ``` -->


<!-- ```{r clust-contrib-merged, fig.width = 12, fig.height = 6} -->
<!-- start_time <- Sys.time() -->
<!-- tibble(clust = integrated@active.ident, -->
<!--            samp = integrated$experiment) %>% -->
<!--     ggplot(aes(x = clust, fill = samp)) + -->
...
<!--               axis.ticks.x = element_blank(), -->
<!--               legend.position = "bottom") -->
<!-- process_time <- stop_the_clock("clust_contrib_merged") -->
<!-- ``` -->
...
<!-- # } -->
<!-- #  -->
<!-- # all_markers <- FindAllMarkers(object = integrated, -->
<!-- #                               logfc.threshold = avg_logFC_thres, -->
<!-- #                               test.use = test_use, -->
<!-- #                               min.pct = min_pct, -->
<!-- #                               only.pos = only_pos, -->
<!-- #                               latent.vars = vars_to_regress, -->
<!-- #                               verbose = TRUE -->
<!-- #                             ) -->
<!-- #  -->
<!-- # if (parallelize) { -->
<!-- #   plan("sequential") -->
<!-- # } -->
<!-- # process_time <- stop_the_clock("find_markers") -->
<!-- ``` -->


<!-- ## Cluster-wise correlation -->

<!-- ```{r calc-avg-exp, warning = FALSE, message = FALSE, eval = TRUE} -->
<!-- start_time <- Sys.time() -->
<!-- all_avgexp_byclust <- tibble() -->
<!-- for (ii in unique(integrated$experiment)) { -->
<!--     temp <- subset(x = integrated, subset = experiment == ii) -->
<!--     temp_avgexp_byclust <- AverageExpression(object = temp, use.raw = TRUE, assays = "RNA", show.progress = FALSE, verbose = FALSE) -->
<!--     temp_avgexp_byclust <- as.data.frame(temp_avgexp_byclust[["RNA"]]) %>% -->
<!--         rownames_to_column("gene") %>% -->
<!--         gather("cluster", "expression", -gene) %>% -->
<!--         add_column(replicate = ii) -->
<!--     all_avgexp_byclust <- bind_rows(all_avgexp_byclust, temp_avgexp_byclust) -->
<!-- } -->
<!-- rm(temp, temp_avgexp_byclust) -->
<!-- invisible(gc()) -->
<!-- process_time <- stop_the_clock("calc_avg_exp") -->
<!-- ``` -->

<!-- ```{r cor-by-clust, warning = FALSE, message = FALSE, eval = TRUE} -->
<!-- start_time <- Sys.time() -->
...
<!--     #t()                                       ## if you want the heatmap to "comparisons x cluster", instead of "cluster x comparisons" -->
<!-- rm(sub_data, all_avgexp_byclust) -->
<!-- invisible(gc()) -->
<!-- process_time <- stop_the_clock("cor_by_clust") -->
<!-- ``` -->

<!-- ```{r set_fig_params} -->
<!-- fig_width <- 5*length(unique(integrated$experiment))-16 -->
<!-- fig_height <- ceiling(length(unique(integrated@active.ident))/3) -->
<!-- ``` -->

<!-- cor_heatmap, fig.width = fig_width, fig.height = fig_height, eval = TRUE -->

<!-- ```{r cor-heatmap, eval = TRUE} -->
<!-- start_time <- Sys.time() -->
<!-- pheatmap(mydata, -->
...
<!--          #fontsize = 16, -->
<!--          angle_col = 90) -->
<!-- process_time <- stop_the_clock("cor_heatmap") -->
<!-- ``` -->
