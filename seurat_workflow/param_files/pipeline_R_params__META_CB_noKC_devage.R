...
# paths etc
raw_path <- file.path("/project/cncb/shared/proj000/analyses/alevin/fru_iso/Alevin_r_6.30_allcb/")
...
gmt_file_path <- "../../../aallen/gene_lists/gmt/D_melanogaster_broadish_neuron_celltype_markers.gmt"
...
qc_plot_cell_type <- c("Cholinergic", "Glutamatergic", "GABAergic", "Monoaminergic", "Kenyon_cell")
# qc_plot_cell_type <- c("Cholinergic", "Glutamatergic", "GABAergic", "Monoaminergic", "Kenyon_cell", "Photoreceptors", "Glia", "Epithelia", "Muscle", "Fat_cells")
...
# Upon iterations of running the pipeline, you may find that some cells that you want to remove.
# Write out a list of the cells that you want to keep, and it can be read in here.
user_cells_remove <- TRUE
# user_cells_remove_file <- "../analyses/cell_lists/New_Meta_cbNeuron_PUBnofacs_noPolIII_v1__cb_neuron_cellids.csv"
user_cells_remove_file <- "../analyses/cell_lists/New_Meta_cbNeuron_PUBnofacs_noPolIII_noKC_v1__cb_neuron_cellids.csv"


neighbours <- 30             # used for RunUMAP and FindNeighbors
n_var_features <- 1000       # number of variable features to use for PCA
...
# what genes to use for reducing/clustering
reduce_by_var_genes <- TRUE              # whether to run variable gene selection
reduce_by_user_gene_list <- FALSE        # whether to only use TFs - if cluster_by_var_genes is false, will use all TFs, if TRUE, will use only varialbe TFs
user_gene_list_file <- "../../../aallen/gene_lists/gene_sets/TFs_RBPs__6_30.tsv"

# any genes you want to make sure are included in the analysis
user_genes_add <- FALSE
genes_add <- c("VAChT", "ChAT", "ChT", "VGlut", "CG46448", "Gad1", "VGAT", "CG14989", "Vmat")
...
curr_pc <- 200                # "ideal" pc

min_pc <- 20                 # min pc to test in range of pcs
max_pc <- 340                 # max pc to test in range of pcs
...
# RunHarmony params
# harmony_group <- c("experiment", "sex")
harmony_group <- c("orig.ident", "experiment", "cell_nuclei", "dev_age")
...
# vars_to_regress <- c("experiment", "nCount_RNA")  #, "sex"
# vars_to_regress <- c("orig.ident", "nCount_RNA")  #, "sex"
# vars_to_regress <- c("tissue", "nCount_RNA")  #, "sex"
vars_to_regress <- NULL                         # c("orig.ident", "nCount_RNA", "percent_mt", "percent_hsp", "sex")
sct_vars_to_regress <- setdiff(vars_to_regress, c("orig.ident", "nCount_RNA", "sex"))


# UMAP settings
# umap_n_neighbors <- neighbours    # just using the neighbours from above
umap_n_epochs <- 1000
umap_min_dist <- 0.01
umap_spread <- 5
...
nn_method <- "annoy"                       # options "annoy", "rann"
nn_eps <- 0                               # used if nn_method <- "rann"
nn_trees <- 200                           # used if nn_method <- "annoy"
...
resolution_test <- c(seq(1, 5, 1), seq(6, 10, 2), seq(15, 25, 5), seq(30, 50, 10))
# resolution_test <- c(1, seq(5, 20, 5), seq(30, 50, 10))
# resolution_test <- c(1, 5, 10, 20, 50)
curr_res <- 20               # "ideal" cluster resolution
...
annotation_cell_type <- c("Cholinergic", "Glutamatergic", "GABAergic", "Monoaminergic", "Kenyon_cell")
# annotation_cell_type <- c("Neuron", "Glia", "Hemocyte", "Epithelia", "Muscle", "Fat_cells")
# annotation_cell_type <- c("Cholinergic", "Glutamatergic", "GABAergic", "Monoaminergic", "Kenyon_cell", "Mechanoreceptors", "Chemoreceptors", "Photoreceptors")
...
subcluster_pcs <- c(100, 100, 100, 60, 20)
subcluster_res <- c(20, 2, 2, 10, 1)
...
wolf_path <- "/project/cncb/aallen/reference_datasets/bulk/GSEXXXXX_Wolf_2024_CX/combined.cells.cpm.updated.xlsx"
