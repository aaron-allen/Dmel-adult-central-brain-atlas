...
<!-- ### Calculate Avg. Module Score -->

<!-- ```{r avg-modulescore} -->
<!-- start_time <- Sys.time() -->
<!-- avg_module_score <- as.data.frame(integrated@meta.data) %>% -->
<!--     select(all_of(curr_res_label), contains("_l2norm")) %>% -->
<!--     select(all_of(curr_res_label), contains(annotation_cell_type, ignore.case = FALSE)) %>% -->
<!--     group_by(.[1]) %>% -->
<!--     gather("Module", "Score", -curr_res_label) %>% -->
<!--     group_by(.[1], Module) %>% -->
<!--     summarise(avg_score = mean(Score)) %>% -->
<!--     mutate(Module = Module %>% str_remove("modulescore_") %>% str_remove("1_l2norm")) -->
<!-- head(avg_module_score) -->
<!-- process_time <- stop_the_clock("avg_modulescore") -->
<!-- ``` -->

<!-- #### List Avg. Module Score -->

<!-- ```{r list-scores} -->
<!-- start_time <- Sys.time() -->
<!-- avg_module_score %>% -->
<!--     spread("Module", "avg_score") %>% -->
<!--     column_to_rownames(var = curr_res_label) -->
<!-- process_time <- stop_the_clock("list_scores") -->
<!-- ``` -->

<!-- #### Plot Avg. Module Score -->

<!-- ```{r plot-heatmap, fig.width = 8, fig.height = 3} -->
<!-- start_time <- Sys.time() -->
<!-- avg_module_score %>% -->
<!--     spread("Module", "avg_score") %>% -->
<!--     column_to_rownames(var = curr_res_label) %>% -->
...
<!--          color = colorspace::diverging_hcl(40, "Blue-Red 3"), -->
<!--          border_color = NA, -->
<!--          cluster_rows = TRUE, -->
<!--          cluster_cols = TRUE, -->
<!--          angle_col = 90) -->
<!-- process_time <- stop_the_clock("plot_heatmap") -->
<!-- ``` -->
...
<!-- ```{r annotate-clusters} -->
<!-- start_time <- Sys.time() -->
<!-- annotation_broad_df <- avg_module_score %>% -->
<!--    spread("Module", "avg_score") %>% -->
<!--     column_to_rownames(var = curr_res_label) -->
<!-- top_annotation_broad <- colnames(annotation_broad_df)[max.col(annotation_broad_df, ties.method = "first")] -->
<!-- my_colnames <- colnames(annotation_broad_df) -->
...
<!--            broad = top_annotation_broad -->
<!--            ) %>% -->
<!--     rownames_to_column(var = curr_res_label) -->
...
<!-- if (annotate_by_K) { -->
<!--     annotation_broad_df <- annotation_broad_df %>% -->
<!--         mutate(broad = if_else(max_value > K_thresh, broad, "unknown")) -->
<!-- } else { -->
<!--     annotation_broad_df <- annotation_broad_df %>% -->
<!--         mutate(broad = if_else(max_value > 0, broad, "unknown")) -->
<!-- } -->
<!-- annotation_broad_df[annotation_broad_df == "unknown"] <- NA -->
<!-- head(annotation_broad_df) -->
<!-- process_time <- stop_the_clock("annotate_clusters") -->
<!-- ``` -->
...
<!-- ```{r get-per-cell-label} -->
<!-- start_time <- Sys.time() -->
<!-- manual_annotation_avg_by_cell <- as.data.frame(integrated@meta.data) %>% -->
<!--     rownames_to_column("cell_id") %>% -->
<!--     select(cell_id, all_of(curr_res_label)) %>% -->
<!--     left_join(annotation_broad_df) -->
<!-- head(manual_annotation_avg_by_cell) -->
<!-- process_time <- stop_the_clock("get_per_cell_label") -->
<!-- ``` -->


<!-- ### Annotate and Plot -->

<!-- ```{r plot-annotation-avg, fig.width = 10, fig.height = 10} -->
<!-- start_time <- Sys.time() -->
<!-- integrated <- AddMetaData(integrated, metadata = manual_annotation_avg_by_cell$broad, col.name = "annotation_broad_cell_type_avg") -->
<!-- DimPlot(object = integrated, -->
<!--         reduction = paste0("umap_", curr_pc, "pcs"), -->
<!--         group.by = "annotation_broad_cell_type_avg", -->
<!--         pt.size = min(4, pt_size), -->
<!--         raster = FALSE) + -->
<!--     NoAxes() + -->
<!--     coord_fixed() -->
<!-- process_time <- stop_the_clock("plot_annotation_avg") -->
<!-- ``` -->

<!-- ### Donut chart -->

<!-- ```{r plot-ann-pie-2, fig.width = 6, fig.height = 6} -->
<!-- ann_use <- "annotation_broad_cell_type_avg" -->
<!-- tibble(Annotations = integrated@meta.data[[ann_use]]) %>% -->
<!--     group_by(Annotations) %>% -->
<!--     dplyr::summarise(count = n()) %>% -->
<!--     ggplot(aes(x = 1, y = count, fill = Annotations)) + -->
