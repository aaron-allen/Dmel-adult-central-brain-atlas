...
<!-- ```{r} -->
<!-- cluster_markers <- read_csv(file = paste0(objects_path, -->
<!--                           "../markers/", -->
<!--                           dataset, "_", -->
<!--                           input_type, "_", -->
<!--                           normalization, "_", -->
...
# Wolf et al 2024 - Central Complex

```{r}
central_complex <- read_xlsx(path = paste0(local_path_mod, wolf_path), 
                             sheet = "combined.cells.cpm.updated", 
                             skip = 0) %>% 
  dplyr::rename("FBgn" = "...1") %>% 
  # dplyr::rename("FBgn" = "X1") %>% 
  select(-gene_class, -gene_symbol, -qRNA_10_pg, -gene_name, -GO_terms, -transcript_ids)
...
central_complex_log <- fbgn_gene %>%
    ...
    unique() %>%
    drop_na() %>%
    inner_join(central_complex, by = "FBgn") %>%
    select(-FBgn, -contains("_CBqRNA")) %>%
    rename("gene" = "gene_symbol_r6.30") %>%
    # gather("cell_type", "expression",  -gene) %>%
    # group_by(cell_type, gene) %>%
    # spread("cell_type", "expression") %>%
...
<!-- ```{r} -->
<!-- merged_cx_slim_corr <- merged_cx_slim_corr %>% mutate(x = str_remove(x, "g")) -->
<!-- merged_cx_slim_corr -->
<!-- ``` -->
...
<!-- # Abruzzi et al 2017 - clock neurons -->



<!-- ```{r} -->
<!-- dn1 <- read_tsv(file = paste0(local_path_mod, abruzzi_path)) %>% -->
<!--     rename("gene" = "X2") -->
<!-- head(dn1) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- dn1_log <- dn1 %>% -->
<!--     gather("sample", "expression", -Symbol, -gene) %>% -->
<!--     group_by(sample, gene) %>% -->
<!--     summarise(sum_exp = sum(expression)) %>% -->
<!--     spread("sample", "sum_exp") %>% -->
<!--     unique() %>% -->
<!--     drop_na() %>% -->
<!--     column_to_rownames("gene") %>% -->
<!--     log1p() %>% -->
...
<!--                                            assay_use = "RNA", -->
<!--                                            slot_use = "data") -->
<!-- merged_dn1_slim_corr <- corr_bulk_sc(avg_exp_mat = merged_avg_exp_by_cluster, bulk_df = dn1_log, assay_use = "RNA") -->
<!-- ``` -->
...
<!-- ```{r fig.width=12, fig.height=6, warning=FALSE, message=FALSE} -->
<!-- Idents(integrated) <- integrated@meta.data[["RNA_snn_res.10"]] -->
<!-- cell_types <- unique(merged_dn1_slim_corr$y) -->
<!-- for (i in seq_along(cell_types)) { -->
<!--     p1 <- plot_corr_gap(object = integrated, -->
<!--                         ident = "RNA_snn_res.10", -->
<!--                         curr_pc = 240, -->
<!--                         corr_df = merged_dn1_slim_corr, -->
<!--                         cell_type = cell_types[[i]], -->
<!--                         y_limits = c(0, 1)) -->
<!--     plot(p1) -->
<!-- } -->
...
<!-- ```{r} -->
<!-- merged_aso_etal_slim_corr <- merged_aso_etal_slim_corr %>% mutate(x = str_remove(x, "g")) -->
<!-- merged_aso_etal_slim_corr -->
<!-- ``` -->
...
<!-- # Shretter et al 2020 - fru subtypes -->
...
<!-- p_list <- list() -->
<!-- genes <- c("fru", "dsx", "EGFP") -->
<!-- for (i in seq_along(genes)) { -->
<!--     p_list[[i]] <- FeaturePlot(object = integrated, features = genes[[i]], pt.size = 0.1, cols = c("lightsteelblue2", "black"), reduction = paste0("umap_", curr_pc, "pcs"), order = TRUE) + coord_fixed() -->
<!-- } -->
...
<!-- genes <- c("fru", "dsx", "EGFP") -->
<!-- for (i in seq_along(genes)) { -->
<!--     p_list[[i]] <- FeaturePlot(object = integrated, features = genes[[i]], pt.size = 0.1, cols = c("lightsteelblue2", "black"), reduction = "umap", order = TRUE) + coord_fixed() -->
<!-- } -->
<!-- plot_grid(plotlist = p_list,  ncol = 3) -->
<!-- ``` -->
