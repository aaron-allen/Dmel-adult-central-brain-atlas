...
parser.add_argument('--aucell_output', default = 'aucell.csv',
...
parser.add_argument('--binarize_threads', default = 20,
...
if args.custom_auc_thresholds is None:
    binary_mtx.to_csv("binary_matrix.csv")
    auc_thresholds.to_csv("aucell_thresholds.csv", header = False, index = True)
else:
    binary_mtx.to_csv("binary_matrix_custom_thresholds.csv")
    auc_thresholds.to_csv("aucell_thresholds.csv", header = False, index = True)
